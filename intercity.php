...
<div class="container">
	<div class="form-group" style="padding:50px" >
		<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" autocomplete="off" enctype="multipart/form-date">
			...
			<button type="submit" name="searchbus" id="searchbus" class="btn btn-default">Search Bus</button>
...
<?php
			...
			if(isset($_POST["searchbus"]))
			{
				...
				if ($result->num_rows > 0)
				{
					...
					echo
						'<input type="radio"  name="busno" id="'.$row["busno"].'" value="'.$row["busno"].'">
					...
					echo
						'<br><br><button type="submit" name="selectBus" id="selectBus" class="btn btn-default">Book Ticket</button>
					</form>';
